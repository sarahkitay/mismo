...
function App() {
  ...
  const parsePath = (path: string): { role: typeof currentRole; page: string; params: Record<string, string> } => {
    ...
    return { role: currentRole === 'EMPLOYEE' ? 'EMPLOYEE' : currentRole === 'CLIENT' ? 'CLIENT' : 'HR', page: currentRole === 'EMPLOYEE' ? 'home' : currentRole === 'CLIENT' ? 'client-dashboard' : 'dashboard', params };
  };
  ...
  // Update active page when role changes
  useEffect(() => {
    const parsed = parsePath(window.location.pathname);
  ...
  const renderEmployeeContent = () => {
    switch (activePage) {
      ...
      default:
        // Check if it's a report detail page
  ...
  return (
    <div className="min-h-screen bg-[var(--mismo-bg)]">
      {/* Toast notifications */}
      <Toaster position="top-right" richColors />
      {/* Preview banner when HR is viewing as employee */}
      {previewUserId && (
      ...
      {/* Main Content */}
      <main className={previewUserId ? 'pt-24 lg:pl-64 min-h-screen relative z-0 pointer-events-auto' : 'pt-16 lg:pl-64 min-h-screen relative z-0 pointer-events-auto'}>
