...
function mapInvestigation(
  ...
  return {
    ...
    investigationType: row.investigation_type ? String(row.investigation_type) : undefined,
    reportSourceType: row.report_source_type as Investigation['reportSourceType'],
    linkedReportIds,
    ...
    linkedPromptResponseId: row.linked_prompt_response_id
      ...
      : undefined,
    employeePreferredContact: row.employee_preferred_contact as Investigation['employeePreferredContact'],
    outcomeSummary: row.outcome_summary ? String(row.outcome_summary) : undefined,
    outcomeRequiresSignature: row.outcome_requires_signature != null ? Boolean(row.outcome_requires_signature) : undefined,
    outcomeSentAt: optDate(row.outcome_sent_at as string | null),
    outcomeEmployeeSignedAt: optDate(row.outcome_employee_signed_at as string | null),
    outcomeEmployeeAgreed:
      row.outcome_employee_agreed == null ? undefined : Boolean(row.outcome_employee_agreed),
    outcomeClassification: row.outcome_classification as Investigation['outcomeClassification'],
    outcomeViewedAt: optDate(row.outcome_viewed_at as string | null),
    findingsRationale: row.findings_rationale ? String(row.findings_rationale) : undefined,
    policyAnalysisNotes: row.policy_analysis_notes ? String(row.policy_analysis_notes) : undefined,
    finalFindingsReport: row.final_findings_report ? String(row.final_findings_report) : undefined,
    legalInvolved: row.legal_involved != null ? Boolean(row.legal_involved) : undefined,
    legalInvolvementNotes: row.legal_involvement_notes ? String(row.legal_involvement_notes) : undefined,
    riskLevel: row.risk_level as Investigation['riskLevel'],
    initialContactNotes: row.initial_contact_notes ? String(row.initial_contact_notes) : undefined,
    nonRetaliationSentAt: optDate(row.non_retaliation_sent_at as string | null),
    pickedUpAt: optDate(row.picked_up_at as string | null),
