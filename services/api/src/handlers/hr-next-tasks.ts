...
import { getDefaultModel, getOpenAIClient } from '../lib/openai.js';
...
function buildTasksFromCounts(counts: {
 payroll: number;
 needsInfo: number;
 wageHour: number;
 yesReview: number;
 ...
 unansweredPrompts?: number;
 openInvestigations?: number;
}, source: HrNextTask['source']): HrNextTask[] {
...
async function fetchDbCounts(orgId: string) {
 ...
 .select('id', { count: 'exact', head: true })
 .eq('org_id', orgId)
 .eq('status', 'PENDING_WAGE_HOUR_REVIEW'),
 supabase
