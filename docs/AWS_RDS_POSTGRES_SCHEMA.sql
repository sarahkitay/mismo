-- =============================================================================
-- MISMO — AWS RDS PostgreSQL Schema (Production Transfer Script)
-- =============================================================================
--
-- Target: Amazon RDS for PostgreSQL 15+ (also compatible with Aurora PostgreSQL)
-- App mapping: src/types/index.ts + src/hooks/useDataStore.ts
--
-- BEFORE RUNNING ON AWS
-- ---------------------
-- 1. Create RDS instance (Multi-AZ recommended for production).
-- 2. Enable encryption at rest + automated backups (≥ 7 days retention).
-- 3. Store credentials in AWS Secrets Manager; inject via Lambda/ECS/Amplify env.
-- 4. Run this script as a migration user with CREATE privileges.
-- 5. Create a separate application role with least privilege (see GRANTS below).
-- 6. Store file binaries in S3 — do NOT persist base64 data URLs in production.
--    Columns named `storage_key` expect s3://bucket/org_id/... object keys.
--
-- TENANCY
-- -------
-- Every business table includes `org_id`. All application queries MUST filter
-- by org_id from the authenticated session (see RLS section at bottom).
--
-- ID STRATEGY
-- -----------
-- Primary keys are TEXT to allow importing existing demo IDs (user-emp-1, etc.).
-- New rows may use:  lower(replace(gen_random_uuid()::text, '-', ''))
--
-- =============================================================================
...
CREATE TYPE report_status AS ENUM (
  ...
  'PENDING_WAGE_HOUR_REVIEW', 'RESOLVED', 'CLOSED'
);
...
CREATE TYPE activity_event_type AS ENUM (
  ...
  'NUDGE_SENT', 'EXPORT_PDF', 'EXPORT_CSV'
);
...
-- Optional: password hash or Cognito sub — wire to AWS Cognito / IAM Identity Center
CREATE TABLE auth_credentials (
  user_id           TEXT PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
  password_hash     TEXT,                    -- bcrypt/argon2 if not using Cognito
  cognito_sub       TEXT UNIQUE,             -- AWS Cognito user pool subject
...
CREATE TABLE report_handling_ledger (
  ...
  file_name       TEXT,
  file_size       BIGINT,
  storage_key     TEXT,                   -- S3 for FILE entries
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE policy_acknowledgements (
  ...
  signature_storage_key TEXT,              -- S3 PNG signature
...
CREATE INDEX idx_metrics_snapshots_org_window ON metrics_snapshots(org_id, window, created_at DESC);
...
-- =============================================================================
-- APPLICATION ROLE & GRANTS (run after creating mismo_app user)
-- =============================================================================
--
-- CREATE ROLE mismo_app LOGIN PASSWORD 'change-me';
...
--
-- =============================================================================
-- ROW LEVEL SECURITY (recommended for multi-tenant isolation)
-- =============================================================================
--
-- Enable after your API sets:  SET app.current_org_id = '<org_id>';
--
-- ALTER TABLE users ENABLE ROW LEVEL SECURITY;
-- CREATE POLICY users_org_isolation ON users
--   USING (org_id = current_setting('app.current_org_id', true));
--
-- Repeat for all org-scoped tables (reports, prompts, investigations, etc.)
--
-- =============================================================================
...
-- =============================================================================
-- LOCALSTORAGE → RDS MIGRATION CHECKLIST
-- =============================================================================
--
-- | localStorage key / entity          | Target table(s)                    |
-- |------------------------------------|------------------------------------|
-- | mismo_session                      | user_sessions                      |
-- | users                              | users + auth_credentials           |
-- | prompts                            | prompts + prompt_target_*          |
-- | deliveries                         | prompt_deliveries                  |
-- | responses                          | prompt_responses                   |
-- | reports (+ nested ledger/checklist)| reports + child tables             |
-- | investigations (+ nested JSON)     | investigations + child tables      |
-- | policies                           | policies                           |
-- | policyAcknowledgements             | policy_acknowledgements            |
-- | announcements                      | announcements                      |
-- | nudges                             | nudges                             |
-- | activities                         | activity_events                    |
-- | reportStatusEvents                 | report_status_events               |
-- | auditLogs                          | audit_logs                         |
-- | wageHourAcknowledgements           | wage_hour_screening_acknowledgements|
-- | checkInGate localStorage           | check_in_deferrals                 |
--
-- Attachments: upload data URLs to S3, store storage_key only.
--
-- =============================================================================
...
--
-- Secrets Manager
--   { "username": "mismo_app", "password": "...", "host": "...", "port": 5432, "dbname": "mismo" }
--
-- S3 (attachments)
--   Bucket: mismo-{env}-files
--   Key pattern: {org_id}/{entity}/{record_id}/{uuid}-{filename}
--   SSE-KMS enabled; block public access
--
-- Optional
--   Amazon Cognito → auth_credentials.cognito_sub
--   EventBridge cron → daily prompt_deliveries generator (replaces client-side useEffect)
